...
class Application(Frame):
    ...
    def createWidget(self):
        # 创建画板
        self.drawCad = Canvas(self, width=win_width,
    ...
    def myline(self, event):
        ...
        self.lastDraw = self.drawCad.create_line(
    ...
    def myRect(self, event):
        self.startDraw(event)
        self.lastDraw = self.drawCad.create_rectangle(
            self.x, self.y, event.x, event.y, outline=self.fgcolor)


    def myPen(self, event):
        self.startDraw(event)
        self.drawCad.create_rectangle(
            self.x, self.y, event.x, event.y, outline=self.fgcolor)
        self.x = event.x
        self.y = event.y
    ...
    def myEarsor(self, event):
        self.startDraw(event)
        self.drawCad.create_rectangle(
            event.x-4, event.y-4, event.x+4, event.y+4, fill=self.bgcolor)
        self.x = event.x
        self.y = event.y
